...
const NavBar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (offset > 50 + TEST_INDICATOR_HEIGHT) {
        // Added indicator height to scroll threshold
        setScrolled(true);
      } else {
        setScrolled(false);
      }
  ...
  const navLinks = [{
    ...
    submenu: [{
      ...
      title: t('nav.projectTrcBochka'),
      href: '/projects/trcbochka'
    }, {
      title: t('nav.projectNewUzbekistan'),
  ...
  const MobileMenu = () => <div className="bg-[#080C16] h-full w-full overflow-auto">
      <nav className="flex flex-col w-full">
        {navLinks.map(link => link.hasSubmenu ? <Collapsible key={link.key} className="w-full">
              ...
              <CollapsibleContent>
                <div className="bg-[#1a1a1a]">
                  {link.submenu?.map(subItem => <SheetClose asChild key={subItem.title}>
                      <a href={subItem.href} className="w-full flex items-center py-2.5 px-6 text-gray-300 hover:text-white hover:bg-white/5 font-benzin text-sm border-b border-white/5">
                        {subItem.title}
                      </a>
                    </SheetClose>)}
                </div>
              </CollapsibleContent>
            </Collapsible> : <SheetClose asChild key={link.key}>
              <a href={link.href} className="w-full flex items-center justify-between py-3 px-4 text-white border-b border-white/10 font-benzin text-base hover:bg-white/5">
                <span>{link.title}</span>
              </a>
            </SheetClose>)}
      </nav>
  ...
  return <>
      ...
      <header className={cn('fixed left-0 right-0 z-50 transition-all duration-300',
    // Calculate top based on the indicator height
    `top-[${TEST_INDICATOR_HEIGHT}px]`,
    // Set top position dynamically

    scrolled ? 'bg-brand-dark/95 backdrop-blur-md shadow-sm py-3' // Original py-3 for scrolled state
    ...
    // Removed mt classes as 'top' is used for vertical positioning of a fixed element
        ...
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between">
            ...
            <a href="/" className="flex-shrink-0">
              <img src="/lovable-uploads/5b8a353d-ebd6-43fe-8f54-7bacba7095ff.png" alt="TOWERUP Logo" className="h-12 md:h-16 w-auto" />
            </a>

            {/* The space between logo and menu items is created by justify-between on the parent div */}
            
            {!isMobile && (/* Navigation Links and Actions - All positioned at the far right */
          <div className="flex items-center justify-end">
                {/* Navigation Links */}
                <nav className="flex items-center">
                  <ul className="flex items-center space-x-6">
                    {navLinks.map(link => link.hasSubmenu ? <li key={link.key} className="relative" onMouseEnter={() => handleMouseEnter(link.key)} onMouseLeave={() => handleMouseLeave(link.key)}>
                          <button className={cn("flex items-center gap-1.5 p-2 font-benzin tracking-wide hover:text-brand-primary transition-colors duration-300 bg-transparent border-0", scrolled ? "text-white" : "text-white")}>
                            <span>{link.title}</span>
                            <ChevronDown className={cn("h-4 w-4 transition-transform duration-300", openDropdowns[link.key] ? "rotate-180" : "")} />
                          </button>
                          ...
                          <a href={link.href} className={cn("tracking-wide hover:text-brand-primary transition-colors duration-300 font-benzin px-3 py-2", scrolled ? "text-white" : "text-white")}>
                            {link.title}
                          </a>
                        </li>)}
                  </ul>
                ...
                <div className="flex items-center gap-4 ml-8">
                  ...
                  <a href="/contact" className={cn("flex items-center gap-2 px-5 py-2.5 rounded-lg bg-brand-primary text-white font-medium", "shadow-lg shadow-brand-primary/20 transform transition hover:-translate-y-0.5 font-benzin")}>
            ...
            {isMobile && <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <SheetContent side="right" className="p-0 w-[85vw] max-w-xs bg-[#080C16] text-white border-l border-white/10">
                  <div className="flex items-center justify-between p-4 border-b border-white/10">
                    <a href="/" className="flex items-center" onClick={() => setIsMenuOpen(false)}>
                      <img src="/lovable-uploads/5b8a353d-ebd6-43fe-8f54-7bacba7095ff.png" alt="TOWERUP Logo" className="h-10 w-auto" />
                    </a>
                  </div>
      ...
      {/* This might be better handled in a layout component or global CSS */}
       <div style={{
      paddingTop: scrolled ? `calc(${TEST_INDICATOR_HEIGHT}px + var(--header-scrolled-height))` : `calc(${TEST_INDICATOR_HEIGHT}px + var(--header-initial-height))`
    }}>
