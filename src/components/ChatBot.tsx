...
const ChatBot: React.FC = () => {
  ...
  const API_KEY = import.meta.env.VITE_GEMINI_API_KEY || "AIzaSyCiNLb7rrA8S-bPmAV5rnIYIDL3XZSONHA"; // Replace placeholder if not using env vars
  ...
  useEffect(() => {
    ...
    if (!hasShownPopup) {
      const timer1 = setTimeout(() => {
        if (!isOpen) {
          // Only show popup if chat is not already open
          setPopupShown(true);
...
*   Признайте, что вы — ИИ-ассистент.
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${API_KEY}`, {
        // Using v1beta and latest flash model
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          systemInstruction: {
            role: "system",
            // Though role is often implied, being explicit can help
            parts: [{
              text: systemPrompt
            }]
          },
  ...
  return <>
        ...
        setPopupShown(false);
            ...
            <div className="fixed bottom-6 right-6 md:right-10 z-[9999]">
                <Button onClick={() => setIsOpen(prev => !prev)} // Toggle open state
      className="rounded-full bg-primary p-3 h-14 w-14 flex items-center justify-center shadow-lg hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" aria-label={isOpen ? "Close chat" : "Open chat"}>
            ...
            {isOpen && <div className="fixed bottom-24 right-6 md:right-10 z-[9998] bg-white rounded-lg shadow-xl w-full max-w-[350px] md:max-w-[400px] h-[500px] flex flex-col border border-gray-200 animate-fade-in">
                    ...
                    <div className="flex-grow p-4 overflow-y-auto bg-gray-50"> {/* Slightly off-white bg */}
                        {messages.map((msg, index) => <div key={index} className={`mb-3 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                return boldPart;
                    ...
                    <div className="p-3 border-t bg-white rounded-b-lg">
                        <div className="flex gap-2">
                            <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} placeholder="Ваш вопрос..." disabled={isLoading} className="flex-grow rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm bg-gray-600" />
