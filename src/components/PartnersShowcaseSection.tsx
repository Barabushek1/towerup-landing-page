...
const PartnersShowcaseSection: React.FC = () => {
    ...
    const startAutoplay = () => {
      ...
      autoplay = setInterval(() => {
        if (document.visibilityState === 'visible') {
          // Only scroll when tab is visible
          emblaApi?.scrollNext();
        }
      }, 3000);
    };
    ...
    queryFn: async () => {
      return getCachedData('partners_showcase', async () => {
        const {
          data,
          error
        } = await supabase.from('partners').select('*').order('name');
        if (error) {
  ...
  return <section className="py-16 md:py-24 relative bg-[#0f0f0f] overflow-hidden"> {/* Adjusted padding */}
            {/* Background elements */}
            ...
            <div className="container mx-auto px-4 sm:px-6"> {/* Adjusted padding */}
                ...
                {/* Loading Skeleton */}
                {isLoading ? <div className="flex justify-center items-center gap-4 overflow-x-hidden pb-4"> {/* Added padding-bottom */}
                        {[...Array(5)].map((_, i) =>
      ...
      <div className="relative px-0 md:px-8"> {/* Removed horizontal padding on mobile */}
            ...
            // Duplicate partners for seamless loop appearance
            <div key={`${partner.id}-${index}`} // Unique key for duplicated items
            ...
            >
                                        <a href={partner.website_url} target="_blank" rel="noopener noreferrer" aria-label={`${t('partners.visitWebsite')} ${partner.name}`} className={cn("group flex items-center justify-center p-4 sm:p-6", "bg-slate-800/40 hover:bg-slate-800/80 rounded-xl border border-slate-700/50", "transition-all duration-300 hover:shadow-lg hover:border-primary/30", "w-full h-24 sm:h-32",
                  ...
                  if (parent) {
                    ...
                    parent.appendChild(textFallback);
                        ...
                        <div className="hidden md:block"> {/* Keep hidden on mobile */}
                            <button onClick={scrollPrev} aria-label={t('partners.prevPartner')} className="absolute left-0 top-1/2 -translate-y-1/2 transform -translate-x-2 bg-primary hover:bg-primary/80 text-white p-2 rounded-full shadow-lg transition-opacity opacity-70 hover:opacity-100 z-10 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-slate-900">
                                <ChevronLeft className="w-5 h-5" />
                            </button>
                            <button onClick={scrollNext} aria-label={t('partners.nextPartner')} className="absolute right-0 top-1/2 -translate-y-1/2 transform translate-x-2 bg-primary hover:bg-primary/80 text-white p-2 rounded-full shadow-lg transition-opacity opacity-70 hover:opacity-100 z-10 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-slate-900">
