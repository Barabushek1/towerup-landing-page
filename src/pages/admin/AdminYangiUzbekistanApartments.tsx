...
interface YangiUzbekistanApartmentUnit {
  id: string;
  floor_number: number;
  area: number;
  room_count: number;
...
const apartmentUnitSchema = z.object({
  ...
  floor_number: z.number().int().min(1, "Минимальный этаж: 1").max(16, "Максимальный этаж: 16"),
...
const AdminYangiUzbekistanApartments: React.FC = () => {
  const queryClient = useQueryClient();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [currentApartmentUnit, setCurrentApartmentUnit] = useState<YangiUzbekistanApartmentUnit | null>(null);
  ...
  const { data: apartmentUnits, isLoading } = useQuery({
    queryKey: ['yangiUzbekistanApartmentUnits'],
    queryFn: async () => {
      ...
      return data as YangiUzbekistanApartmentUnit[];
    }
  });
  ...
  const createApartmentUnitMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['yangiUzbekistanApartmentUnits'] });
  ...
  const handleEdit = (unit: YangiUzbekistanApartmentUnit) => {
  ...
  const handleDelete = (unit: YangiUzbekistanApartmentUnit) => {
    setCurrentApartmentUnit(unit);
    ...
    <AdminLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold">Управление квартирами - Yangi Uzbekistan</h1>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle>Список квартир - Yangi Uzbekistan</CardTitle>
...
export default AdminYangiUzbekistanApartments;
