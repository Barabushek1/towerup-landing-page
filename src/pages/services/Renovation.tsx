...
const Renovation: React.FC = () => {
  return <ServicePageLayout title="Ремонт" description="Профессиональные ремонтные работы любой сложности от компании TOWERUP" breadcrumb="Услуги / Ремонт" headerImage="https://i.ibb.co/LdMZYVgg/Generated-Image-May-20-2025-1-01-AM.jpg">
      {/* Hero Section */}
          ...
          <AnimatedSection className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div variants={fadeIn} className="relative">
              <div className="aspect-square overflow-hidden rounded-2xl border-2 border-white/10 shadow-2xl">
                <img src="/lovable-uploads/fd0b85fc-caff-4e79-8acf-8c3e7ce81787.png" alt="Ремонтные работы TOWERUP" className="w-full h-full object-cover" />
              </div>
              <div className="absolute -bottom-6 -left-6 w-2/3 h-2/3 rounded-xl overflow-hidden border-2 border-white/10 shadow-lg">
                <img alt="Отделочные работы TOWERUP" className="w-full h-full object-cover" src="/lovable-uploads/72f57f29-b8e1-41f2-99e7-39285626244a.jpg" />
              </div>
            </motion.div>
          </AnimatedSection>
        </div>
              ...
              <motion.div variants={fadeIn} className="order-1 md:order-2">
                <div className="sticky top-24 space-y-6">
                  <div className="aspect-[3/4] overflow-hidden rounded-xl border border-white/10 shadow-xl">
                    <img src="/lovable-uploads/32c3e8f6-2da4-474c-904f-fd321d91e87e.png" alt="Процесс ремонта TOWERUP" className="w-full h-full object-cover" />
                  </div>
                  
                  <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                    <h3 className="text-xl font-semibold text-white mb-4">Гарантии качества</h3>
                    <ul className="space-y-3">
                      {["Официальный договор с четкими сроками и ответственностью", "Гарантия до 3-х лет на все выполненные работы", "Использование только сертифицированных материалов", "Команда профессиональных мастеров с опытом от 10 лет"].map((item, index) => <li key={index} className="flex items-start">
      ...
      <section className="py-20 md:py-24 bg-[#161616]">
        <div className="container mx-auto px-6">
          <AnimatedSection>
            ...
            }].map((testimonial, index) => <motion.div key={index} variants={fadeIn} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6">
                  <div className="flex flex-col h-full">
                    <div className="mb-4">
                      <div className="flex items-center space-x-1 text-primary">
                        {[1, 2, 3, 4, 5].map((_, i) => <svg key={i} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 fill-current" viewBox="0 0 20 20">
