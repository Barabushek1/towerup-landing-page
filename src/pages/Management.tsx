...
import { StaffMember, fetchExecutives, fetchDepartmentHeads } from '@/utils/staff-helpers';
import { Loader2 } from 'lucide-react';

const Management: React.FC = () => {
  const [executives, setExecutives] = useState<StaffMember[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [executivesData, departmentHeadsData] = await Promise.all([
          fetchExecutives(),
          fetchDepartmentHeads()
        ]);
        
        setExecutives(executivesData);
        setDepartmentHeads(departmentHeadsData);
      } catch (error) {
        console.error('Error fetching management data:', error);
        // Use default data if database fetch fails
        setExecutives([
          { id: "1", name: "Алексей Петров", position: "Генеральный директор", bio: "Более 20 лет опыта в строительной индустрии. Под его руководством компания успешно реализовала более 30 крупных проектов.", email: "petrov@towerup.ru", phone: "+7 (903) 123-45-67" },
          { id: "2", name: "Елена Смирнова", position: "Финансовый директор", bio: "Магистр финансов МГУ. Отвечает за финансовую стратегию компании, управление рисками и оптимизацию затрат.", email: "smirnova@towerup.ru", phone: "+7 (903) 765-43-21" },
          { id: "3", name: "Иван Соколов", position: "Технический директор", bio: "Инженер-строитель с международным опытом работы. Внедрил современные технологии строительства, повысившие эффективность на 35%.", email: "sokolov@towerup.ru", phone: "+7 (903) 222-33-44" },
          { id: "4", name: "Мария Иванова", position: "Директор по персоналу", bio: "Кандидат психологических наук. Создала систему подбора и обучения персонала, позволившую снизить текучесть кадров на 40%.", email: "ivanova@towerup.ru", phone: "+7 (903) 555-66-77" }
        ]);
        setDepartmentHeads([
          { id: "5", name: "Дмитрий Волков", position: "Руководитель отдела продаж", bio: "10 лет опыта в сфере недвижимости. Под его руководством объем продаж вырос на 65% за последние 3 года." },
          { id: "6", name: "Анна Кузнецова", position: "Руководитель юридического отдела", bio: "Более 15 лет юридической практики. Обеспечивает безупречную правовую поддержку всех проектов компании." },
          { id: "7", name: "Сергей Новиков", position: "Главный архитектор", bio: "Обладатель международных наград в области архитектуры. Создал уникальный узнаваемый стиль проектов компании." },
          { id: "8", name: "Ольга Морозова", position: "Руководитель отдела маркетинга", bio: "Разработала стратегию бренда, которая повысила узнаваемость компании на 78% за два года." }
        ]);
      } finally {
        setIsLoading(false);
      }
                  ...
                  <TabsContent value="executives" className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {executives.map((person, index) => (
                        <Card key={person.id || index} className="border border-gray-700/30 bg-[#222] hover:bg-[#272727] transition-all duration-300">
